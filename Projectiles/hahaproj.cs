    ...
    public class hahaproj : ModProjectile
    {
        ...
        public override void AI()
        {
            ...
            if (Projectile.ai[0] >= 0f && Projectile.ai[0] < 200f)
            {
                ...
                {
                    float num544 = 20f;
                    Vector2 vector40 = new Vector2(Projectile.Center.X + (float)Projectile.width * 0.5f, Projectile.position.Y + (float)Projectile.height * 0.5f);
                ...
                else
                {
                    ...
                    for (int num549 = 0; num549 < 200; num549++)
                    {
                        ...
                        {
                            float num550 = Main.npc[num549].Center.X + (float)(Main.npc[num549].width / 2);
                            float num551 = Main.npc[num549].Center.Y + (float)(Main.npc[num549].height / 2);
                            float num552 = Math.Abs(Projectile.Center.X + (float)(Projectile.width / 2) - num550) + Math.Abs(Projectile.Center.Y + (float)(Projectile.height / 2) - num551);
